    ...
    <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow">
        <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row border border-[#19140015] dark:border-[#3E3E3A]/40 rounded-xl overflow-hidden shadow-lg">
            <div class="text-[13px] leading-[20px] flex-1 p-6 pb-6 lg:p-20 lg:pb-16 bg-white dark:bg-[#161615] dark:text-[#EDEDEC]">
                <h1 class="mb-1 font-bold text-lg text-gray-900 dark:text-white">{{ __('Log in to Store13') }}</h1>
                <p class="mb-6 text-[#706f6c] dark:text-[#A1A09A]">{{ __('Welcome back! Enter your credentials to access your library.') }}</p>
                @if ($errors->any())
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-[11px] text-gray-400 mb-1">{{ __('Email Address') }}</label>
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="admin@store.com" class="w-full px-3 py-1.5 rounded-md border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-xs focus:outline-none focus:border-[#f53003] transition-colors">
                    </div>
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="block text-[11px] text-gray-400">{{ __('Password') }}</label>
                        </div>
                        <input type="password" name="password" required placeholder="password" class="w-full px-3 py-1.5 rounded-md border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-xs focus:outline-none focus:border-[#f53003] transition-colors">
                    </div>
                    <div class="flex items-center gap-2 pt-1">
                        <input type="checkbox" name="remember" id="remember" class="rounded-md border-gray-300 text-[#f53003] focus:ring-[#f53003] w-3.5 h-3.5">
                        <label for="remember" class="text-[11px] text-gray-500">{{ __('Remember session') }}</label>
                    </div>
                    <div class="pt-2">
