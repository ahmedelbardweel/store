...
@section('title', __('Secure Checkout') . ' - Store13')

@section('content')
    <h1 class="text-xl sm:text-2xl font-black tracking-tight text-gray-900 dark:text-white mb-5 sm:mb-6">{{ __('Secure Checkout') }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-amber-50 dark:bg-amber-950/20 border border-amber-300 dark:border-amber-700 rounded-xl p-4 sm:p-5 space-y-3">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-xs text-amber-800 dark:text-amber-400 uppercase tracking-wider">{{ __('Test Mode') }}</span>
                    </div>
                    <button type="button" id="fill-test-card" class="text-[10px] px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-bold transition-all cursor-pointer shrink-0 flex items-center gap-1">
            ...
            <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl p-6 custom-shadow space-y-6">
                <h2 class="font-bold text-sm text-gray-900 dark:text-white uppercase tracking-wider">{{ __('Payment Details') }}</h2>
                <form id="checkout-form" action="{{ route('checkout.process') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">{{ __('Cardholder Name') }}</label>
                        <input type="text" id="card_name" name="card_name" required placeholder="Ahmed Test" class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-sm focus:outline-none focus:border-[#f53003] transition-colors">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">{{ __('Card Number') }}</label>
                        <input type="text" id="card_number" name="card_number" required maxlength="19" placeholder="4242-4242-4242-4242" class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-sm focus:outline-none focus:border-[#f53003] transition-colors">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs text-gray-400 mb-1">{{ __('Expiration Date') }}</label>
                            <input type="text" id="card_expiry" name="card_expiry" required maxlength="5" placeholder="12/28" class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-sm focus:outline-none focus:border-[#f53003] transition-colors">
                        </div>
                        <div>
                            <label class="block text-xs text-gray-400 mb-1">CVV</label>
                            <input type="text" id="card_cvv" name="card_cvv" required maxlength="4" placeholder="123" class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 text-sm focus:outline-none focus:border-[#f53003] transition-colors">
                        </div>
                    </div>
                    <div class="pt-4">
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl p-6 custom-shadow space-y-6">
                <h2 class="font-bold text-sm text-gray-900 dark:text-white uppercase tracking-wider">{{ __('Order Summary') }}</h2>
                    ...
                    @foreach ($items as $item)
                        ...
                        <div class="flex items-center justify-between py-3">
                            <div class="min-w-0 pr-4">
                                ...
                                <span class="text-[10px] text-gray-400 block">{{ __($p->category->name) }}</span>
                            </div>
...
<script>
    ...
    if (cardInput) {
        cardInput.addEventListener('input', function (e) {
            ...
            this.value = parts.length > 0 ? parts.join('-') : val;
        });
    }
    ...
    if (fillBtn) {
        fillBtn.addEventListener('click', function () {
            ...
            this.textContent = isRtl ? '✓ تم التعبئة!' : '✓ Filled!';
            this.classList.replace('bg-amber-500', 'bg-emerald-500');
            setTimeout(() => {
                this.textContent = isRtl ? '✦ تعبئة تلقائية' : '✦ Auto-Fill';
