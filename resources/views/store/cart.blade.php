...
@section('content')
    ...
    @if (empty($items))
        <div class="text-center py-16 bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] custom-shadow">
            ...
            <a href="{{ route('home') }}" class="mt-6 inline-block text-xs bg-[#f53003] text-white px-4 py-2 rounded-[15px] font-semibold hover:bg-red-700 transition-colors">
                Return to Store
            </a>
        </div>
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div class="lg:col-span-2 space-y-3">
                @foreach ($items as $item)
                    ...
                    <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] p-3 sm:p-4 custom-shadow flex items-center gap-3 sm:gap-4">
                        <img src="{{ $p->thumbnail_url }}" alt="{{ $p->name }}" class="w-14 h-14 sm:w-16 sm:h-16 object-cover rounded-[18px] border border-gray-100 dark:border-zinc-800 shrink-0">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] p-6 custom-shadow space-y-6">
                    ...
                    <!-- Checkout Button -->
                    <a href="{{ route('checkout.index') }}" class="block w-full text-center text-xs py-3 bg-[#f53003] hover:bg-red-700 text-white rounded-[18px] font-bold transition-all">
                        Proceed to Checkout
                    </a>
                </div>
                ...
                <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] p-6 custom-shadow space-y-4">
                    ...
                    @if ($coupon)
                        <div class="p-2 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-[12px] flex items-center justify-between text-xs">
                    ...
                    @else
                        <form action="{{ route('cart.coupon') }}" method="POST" class="flex gap-2">
                            ...
                            <input 
                                ...
                                class="flex-1 px-3 py-1.5 rounded-[12px] border border-gray-200 dark:border-zinc-800 text-xs bg-gray-50 dark:bg-zinc-900 focus:outline-none focus:border-[#f53003] uppercase"
                            >
