...
@section('title', __('Shopping Cart') . ' - Store13')

@section('content')
    <h1 class="text-2xl font-black tracking-tight text-gray-900 dark:text-white mb-6">{{ __('Your Shopping Cart') }}</h1>

    @if (empty($items))
        <div class="text-center py-16 bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl custom-shadow">
            <svg class="w-12 h-12 text-gray-300 dark:text-zinc-700 mx-auto mb-3" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
            </svg>
            <h3 class="mt-4 text-sm font-semibold text-gray-900 dark:text-white">{{ __('Your Cart is Empty') }}</h3>
            <p class="mt-1 text-xs text-gray-400">{{ __('Add digital games, apps, or music packs to your cart to checkout.') }}</p>
            <a href="{{ route('home') }}" class="mt-6 inline-block text-xs bg-[#f53003] text-white px-4 py-2 rounded-lg font-semibold hover:bg-red-700 transition-colors">
                {{ __('Return to Store') }}
            </a>
        </div>
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                @foreach ($items as $item)
                    ...
                    <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl p-3 sm:p-4 custom-shadow flex items-center gap-3 sm:gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <span class="text-[10px] text-gray-400 block">v{{ $p->version ?? '1.0' }} • {{ $p->file_size ?? __('N/A') }}</span>
                        </div>
                        <div class="flex items-center gap-3 sm:gap-6 shrink-0">
                            <span class="font-black text-sm text-gray-900 dark:text-white">{{ $p->formatted_price }}</span>
                            <form action="{{ route('cart.remove', $p) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-xs text-gray-400 hover:text-[#f53003] transition-colors cursor-pointer p-1" title="{{ __('Remove') }}">✕</button>
                            </form>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="space-y-6">
                <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl p-6 custom-shadow space-y-6">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-zinc-800/80 text-xs">
                        ...
                        <div class="flex items-center justify-between py-3 text-sm">
                            <span class="font-bold text-gray-900 dark:text-white">{{ __('Total Amount') }}</span>
                ...
                <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-xl p-6 custom-shadow space-y-4">
                    <h3 class="font-bold text-xs text-gray-900 dark:text-white uppercase tracking-wider">{{ __('Coupon Code') }}</h3>
                    @if ($coupon)
                        <div class="p-2 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-md flex items-center justify-between text-xs">
                            <span class="font-bold text-emerald-800 dark:text-emerald-400">{{ $coupon }} {{ __('Applied') }}</span>
                            <form action="{{ route('cart.coupon.remove') }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-[10px] text-red-500 hover:underline cursor-pointer">{{ __('Remove') }}</button>
                            </form>
                        </div>
                    @else
                        <form action="{{ route('cart.coupon') }}" method="POST" class="flex gap-2">
                            @csrf
                            <input type="text" id="coupon-input" name="coupon" placeholder="{{ __('Enter coupon code...') }}" class="flex-1 px-3 py-1.5 rounded-md border border-gray-200 dark:border-zinc-800 text-xs bg-gray-50 dark:bg-zinc-900 focus:outline-none focus:border-[#f53003] uppercase">
                        ...
                        <div class="pt-2 border-t border-gray-100 dark:border-zinc-800/50 space-y-2">
                            <p class="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">{{ __('Available Test Coupons - click to use:') }}</p>
                            <div class="flex flex-wrap gap-2">
                                @php
                                    $coupons = [
                                        ['code' => 'SAVE10',  'desc' => __('$10 off'),    'color' => 'blue'],
                                        ['code' => 'SAVE20',  'desc' => __('$20 off'),    'color' => 'purple'],
                                        ['code' => 'WELCOME', 'desc' => __('$5 off'),     'color' => 'green'],
                                        ['code' => 'FREE100', 'desc' => __('$100 off'),   'color' => 'red'],
                                    ];
                                @endphp
                                @foreach ($coupons as $c)
                                    <button type="button" onclick="document.getElementById('coupon-input').value='{{ $c['code'] }}'"
