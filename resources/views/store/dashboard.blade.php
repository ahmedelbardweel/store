...
@section('content')
    ...
    <div class="grid grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] p-3 sm:p-5 custom-shadow">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @foreach ($orders as $order)
                @foreach ($order->items as $item)
                    @php 
                        ...
                        $p = $item->product;
                    @endphp
                    <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] p-5 custom-shadow flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        
                        <div class="flex items-center gap-4 min-w-0">
                            <img src="{{ $p->thumbnail_url }}" alt="{{ $p->name }}" class="w-16 h-16 object-cover rounded-[18px] border border-gray-100 dark:border-zinc-800">
                            <div class="min-w-0">
                                ...
                                @if ($item->license_key)
                                    <div class="mt-2 p-1.5 bg-gray-50 dark:bg-zinc-900 border border-gray-150 dark:border-zinc-800 rounded-[12px] text-[10px] font-mono select-all inline-block">
                        ...
                        <div class="flex items-center gap-4 justify-between sm:justify-end border-t sm:border-t-0 border-gray-100 pt-3 sm:pt-0">
                            ...
                            @if ($order->canDownload() && $item->download_count < $item->max_downloads)
                                <a 
                                    ...
                                    class="text-xs px-4 py-2 bg-[#f53003] hover:bg-red-700 text-white rounded-[18px] font-bold transition-all"
                                >
                                    Download File
                                </a>
                            @else
                                <span class="text-xs px-4 py-2 bg-gray-100 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 text-gray-400 rounded-[18px] cursor-not-allowed">
                                    Disabled
                                </span>
                            @endif
                        </div>
            ...
            @if (!$hasItems)
                <div class="text-center py-16 bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[20px] custom-shadow">
                        ...
                        @foreach ($orders as $o)
                            <div class="pt-4 first:pt-0 space-y-2">
                                ...
                                <div class="flex items-center justify-between text-[10px] text-gray-400">
                                    ...
                                    <span class="px-1.5 py-0.5 rounded-[12px] text-[8px] font-bold uppercase tracking-wider {{ $o->downloads_revoked ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600' }}">
