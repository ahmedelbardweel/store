...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 pb-28 lg:pb-0">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[15px] p-6 custom-shadow space-y-4">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <span class="px-2 py-0.5 rounded-[8px] text-[10px] font-semibold uppercase tracking-wider bg-red-50 dark:bg-red-950/20 text-[#f53003] dark:text-[#FF4433]">
            ...
            <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[15px] overflow-hidden custom-shadow">
                ...
                @if ($product->category->slug === 'music')
                    ...
                    <div class="p-8 bg-zinc-50 dark:bg-zinc-900/50 flex flex-col items-center justify-center space-y-4 border-b border-gray-150 dark:border-zinc-800">
                        <div class="p-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-[15px] text-[#f53003] dark:text-[#FF4433] shadow-sm">
                        ...
                        <div class="border-t border-gray-150 dark:border-zinc-800 pt-6 space-y-4">
                            ...
                            <form action="{{ route('products.review', $product) }}" method="POST" class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <select name="rating" class="text-xs p-2 rounded-[8px] border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 w-24">
                                ...
                                <div>
                                    ...
                                    <textarea name="comment" rows="3" class="text-xs p-2 rounded-[8px] border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 w-full" placeholder="Share your experience downloading this product..."></textarea>
                                </div>
                                <button type="submit" class="text-xs px-4 py-2 bg-[#f53003] text-white rounded-[10px] font-semibold cursor-pointer">
                                    Submit Review
                                </button>
                            </form>
                        </div>
        ...
        <div class="hidden lg:block space-y-6">
            ...
            <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[15px] p-6 custom-shadow space-y-6 sticky top-20">
                ...
                <div class="space-y-3">
                    @if ($product->is_free)
                        ...
                        @else
                            <a href="{{ route('login') }}" class="block w-full text-center text-xs py-3 bg-[#111111] dark:bg-white text-white dark:text-black rounded-[12px] font-bold hover:opacity-90 transition-all">
                                Log in to Download
                            </a>
                        @endauth
                    @else
                ...
                @if ($product->is_free)
                    @auth
                        <form action="{{ route('checkout.process') }}" method="POST">
                            ...
                            <input type="hidden" name="card_cvv" value="000">
                            @php session()->put('cart', [$product->id => 1]); @endphp
                            <button type="submit" class="w-full text-center text-sm py-3 bg-[#f53003] hover:bg-red-700 text-white rounded-[15px] font-bold transition-all cursor-pointer">
                                Download Free
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="block w-full text-center text-sm py-3 bg-[#111111] dark:bg-white text-white dark:text-black rounded-[15px] font-bold hover:opacity-90 transition-all">
                            Log in to Download
                        </a>
                    @endauth
                @else
