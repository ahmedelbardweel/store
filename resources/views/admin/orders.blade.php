...
@section('content')
    ...
    <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[15px] p-4 custom-shadow mb-6 flex flex-wrap gap-4 items-center justify-between">
        <form action="" method="GET" class="flex flex-wrap gap-3 items-center w-full sm:w-auto">
            <select name="status" class="px-3 py-1.5 rounded-[8px] border border-gray-200 dark:border-zinc-800 text-xs bg-gray-50 dark:bg-zinc-900">
                    ...
                    @foreach ($orders as $order)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-zinc-900/30">
                            ...
                            <td class="p-4 text-center font-bold">
                                <span class="px-1.5 py-0.5 rounded-[8px] text-[8px] font-bold uppercase tracking-wider {{ $order->downloads_revoked ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600' }}">
                            ...
                            <td class="p-4 text-right">
                                <form action="{{ route('admin.orders.revoke', $order) }}" method="POST">
                                    ...
                                    <button 
                                        type="submit" 
                                        class="text-xs px-2.5 py-1 rounded-[8px] border {{ $order->downloads_revoked ? 'border-emerald-500 text-emerald-500 hover:bg-emerald-500 hover:text-white' : 'border-red-500 text-red-500 hover:bg-red-500 hover:text-white' }} font-semibold transition-all cursor-pointer"
                                    >
