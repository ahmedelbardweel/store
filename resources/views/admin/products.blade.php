...
@section('content')
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.products.create') }}" class="text-xs px-4 py-2.5 bg-[#f53003] hover:bg-red-700 text-white rounded-[12px] font-bold transition-all">
            + Add New Product
        </a>
    </div>

    <!-- Filter Bar -->
    <div class="bg-white dark:bg-[#161615] border border-gray-200 dark:border-zinc-800 rounded-[15px] p-4 custom-shadow mb-6 flex flex-wrap gap-4 items-center justify-between">
        <form action="" method="GET" class="flex flex-wrap gap-3 items-center w-full sm:w-auto">
            <input 
                ...
                class="px-3 py-1.5 rounded-[8px] border border-gray-200 dark:border-zinc-800 text-xs bg-gray-50 dark:bg-zinc-900 focus:outline-none focus:border-[#f53003]"
            >
            <select name="category" class="px-3 py-1.5 rounded-[8px] border border-gray-200 dark:border-zinc-800 text-xs bg-gray-50 dark:bg-zinc-900">
                    ...
                    @foreach ($products as $p)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-zinc-900/30">
                            ...
                            <td class="p-4 flex items-center gap-3">
                                <img src="{{ $p->thumbnail_url }}" class="w-10 h-10 object-cover rounded-[10px] border border-gray-100 dark:border-zinc-800">
                                <div class="min-w-0">
                            ...
                            <td class="p-4 text-center">
                                <span class="px-1.5 py-0.5 rounded-[8px] text-[9px] font-bold uppercase tracking-wider {{ $p->is_featured ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400' }}">
